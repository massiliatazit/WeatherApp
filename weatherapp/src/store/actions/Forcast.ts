...
export const getweather = (lat: string,lon:string,part:string):ThunkAction<void,RootState,null,WeatherAction>=>{
    return async dispatch=>{
        try {
            const response= await fetch(`https://api.openweathermap.org/data/2.5/onecall?lat=${lat}&lon=$${lon}&exclude=${part}&appid=${process.env.REACT_APP_API_KEY}`)
            if(!response.ok){
